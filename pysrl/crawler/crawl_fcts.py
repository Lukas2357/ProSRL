...
def make_ordered_learnpages() -> dict:
    """Make a list of learn pages ordered according to lenvi workflow

    Returns:
        dict: A dict of sorted learn pages according to lenvi workflow
    ...
    pages = load_learn_pages(file="learn_pages_crawled")
    new_pages = {}

    df = pd.DataFrame(pages).T

    links = (df[df.Title.isna() & df.Topic.isna() & (
            (df.Type == "Üben Grundwissen") | (
                df.Type == "Üben Erweitertes Wissen"))].Link
             )
    ...
    for link in links:
        ...
        table_entries = get_browser_soup(br).find_all("td")

        table = pd.DataFrame(
            np.zeros([len(table_entries) // 3, 3]),
            columns=["niedrig", "mittel", "schwer"],
        )

        for idx, table_entry in enumerate(table_entries):
            title = table_entry.find("a")
            table.iloc[idx // 3, idx % 3] = (
                title.decode_contents() if title else ""
            )
        ...
        ordered_pages = {}
        for idx, table_break in enumerate(table_breaks[1:]):
            c_table = table.iloc[table_breaks[idx]: table_break]
            for col in c_table.columns:
                for entry in c_table[col]:
                    for label, page in pages.items():
                        if page["Title"] == entry:
                            if entry not in ordered_pages:
                                ordered_pages[label] = page

        new_pages = {}
        for label, page in pages.items():
            if page["Title"] != list(ordered_pages.values())[0]["Title"]:
                new_pages[label] = page
            else:
                break

        new_pages = new_pages | ordered_pages

        for label, page in pages.items():
            if label not in new_pages.keys():
                new_pages[label] = page

        pages = deepcopy(new_pages)

    ordered_pages = {}

    def check(cpage, ccategory, ctype):
        return cpage["Category"] == ccategory and cpage["Type"] == ctype

    for category in categories:
        bk_pages_grund = {
            label: page
            for label, page in new_pages.items()
            if check(page, category, "Erarbeiten Grundwissen")
        }
        bk_pages_erweitert = {
            label: page
            for label, page in new_pages.items()
            if check(page, category, "Erarbeiten Erweitertes Wissen")
        }
        u_pages_grund = {
            label: page
            for label, page in new_pages.items()
            if check(page, category, "Üben Grundwissen")
        }
        u_pages_erweitert = {
            label: page
            for label, page in new_pages.items()
            if check(page, category, "Üben Erweitertes Wissen")
        }
        t_pages_grund = {
            label: page
            for label, page in new_pages.items()
            if check(page, category, "Testen Grundwissen")
        }
        t_pages_erweitert = {
            label: page
            for label, page in new_pages.items()
            if check(page, category, "Testen Erweitertes Wissen")
        }
