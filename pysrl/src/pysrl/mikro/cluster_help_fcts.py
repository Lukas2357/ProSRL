...
from helper import save_data, load_input


def get_folder_path(alg: str, features: list) -> str:
    """Get folder path to store results of data and plots
...
def save_cluster_result(combis: list, result: dict, path: str, user: pd.Series):
    """Save the result data of a clustering process
...
def generate_model(alg: str, n_cluster: int, data: pd.DataFrame) -> any:
    ...
    Args:
        ...
        data (pd.DataFrame): The data to use for fitting afterwards
