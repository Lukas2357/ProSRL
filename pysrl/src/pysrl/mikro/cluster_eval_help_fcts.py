...
from cluster_fcts import generic_clustering
from cluster_plot_help_fcts import init_subplots_plot, \
    get_current_axis, save_figure


def _get_labels(df: pd.DataFrame, features: list, n_cluster: int, left_out: int,
                dim=2, alg='k_means', scale='MinMax') -> dict[dict]:
    """Perform clustering and get labels with index of data point
    ...
    Returns:
        dict[dict]: For each feature combination a dict of indices of data
...
def cross_validation(df: pd.DataFrame, features: list, left_outs=None,
    ...
    Args:
        ...
        features (list): The features to use
        left_outs (range): The range of numbers of data points to left out
...
def elbow_plot(df: pd.DataFrame, features: list, dim=2, max_cluster=5,
    ...
    save_figure(save, dpi, os.path.join(path, "elbow"))
...
def silhouette_analysis(df: pd.DataFrame, features: list, dim=2,
                        ...
                        save=False, path='', dpi=300) -> pd.DataFrame:
    ...
    for idx, feature in enumerate(combis):
        ...
        for i, k in enumerate(n_clusters):
            ...
            c_ax1 = ax[idx][2*i] if isinstance(ax[0], Iterable) else ax[2*i]
            c_ax2 = ax[idx][2*i+1] if isinstance(ax[0], Iterable) else ax[2*i+1]
            ...
            c_ax1.axvline(avg_score, linestyle='--', linewidth=2, color='green')
            c_ax1.set_yticks([])
            c_ax1.set_xlim([-1, 1])
            ...
            if dim == 2:
                sns.scatterplot(x=c_values.iloc[:, 0], 
                                y=c_values.iloc[:, 1], ax=c_ax2, 
                                hue=c_labels, palette="tab10")
                c_ax2.set_ylabel(feature[1])
            else:
                data = pd.concat([c_values.iloc[:, 0], pd.Series(c_labels)],
                                 axis=1)
    ...
    save_figure(save, dpi, os.path.join(path, "silhouette"))

    plt.show()
    
    result = pd.DataFrame(result, columns=n_clusters, index=labels[key].keys())
    
    plt.figure(figsize=(8, 2+0.8*len(result.index)))
    sns.heatmap(result, annot=True, fmt='.3f')
    plt.ylabel('')
    plt.tight_layout()

    save_figure(save, dpi, os.path.join(path, "silhouette_summary"))
