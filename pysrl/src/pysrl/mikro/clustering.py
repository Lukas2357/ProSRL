...
from cluster_eval_help_fcts import elbow_plot, silhouette_analysis
...
def do_cluster(data: pd.DataFrame, features: list, n_clusters=(2, ), dim=1, 
               alg='k_means', scaler='Standard', plot=True, pca=True,
               lineplots=True, abline=False, c_thresh=2, learntypes=None, 
               n_bins=20, dpi=100, show_cluster_of=None, save=True,
               elbow=True, silhouette=True):
    ...
    if alg == 'hierarchy':
        ...
        if lineplots:
            print('-> Generate Lineplots... to avoid set an.create_lineplots='
                  'False ...')
        ...
        for n in n_clusters:
            ...
            if alg == 'k_means':
                ...
                print(f'-> Performing {alg} clustering ... to see cluster '
            ...
            if plot:
                if dim < 3:
                    ...
                    for entry in res_data:
                        ...
                        print('-> Generate scatterplots for each possible '
                              'feature combination ...')
        ...
        if silhouette:
            if dim < 3:
                file_path = os.path.join(folder_path, f"{dim}D")
                print('-> Generate silhouette_analysis plots... to avoid set '
                      'an.silhouette=False ...')
