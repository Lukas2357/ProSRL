...
from transform_help_fcts import get_categories, \
    get_learn_types
...
def learn_types_lineplots(cluster=pd.Series(dtype=str), save=True, dpi=120,
    ...
    for c_idx in indices:
        ...
        p = path if file_extension == 'overwrite' else os.path.join(path, file)
        save_figure(save=save, path=p, dpi=dpi, fig=fig)
...
def plot_pca(pca: PCA, components: pd.DataFrame, clusters: pd.DataFrame,
    ...
    Args:
        ...
        clusters (pd.DataFrame): The clusters of the data points
    ...
    if center is not None and len(center.index) != 0:
        ...
        fig['data'][-1]['name'] = 'Cluster Zentren'
