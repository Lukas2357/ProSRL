...
from ..preparer.prep_fcts import scaled_pca
from ..preparer.prep_plot_fcts import plot_pca, \
    learn_types_lineplots


def load_prep_data():
    """Loads the prepared data from csv file
...
def generic_clustering(df: pd.DataFrame, features: list, n_cluster=3, dim=2,
    ...
    Args:
        ...
        path (str): Path to where result data will be saved to
        save (bool): Whether to save the result data
    ...
    for combi in combis:
        ...
        if len(data[np.isnan(data)]) > 0:
            warnings.warn(f"Nan values in {tag} set to 0 while fitting model!")
...
def do_pca(data: list, clusters: pd.DataFrame, file_path: str, centers=None,
    ...
    """Perform PCA on given data and plot the results
